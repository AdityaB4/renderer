...
void makePyramid(double x, double y, Scene &scene);
...
// Define materials.
    ...
    black.k_a = Color(0.2f, 0.2f, 0.2f);
    // black.k_a = black.k_d;
    ...
    // Insert into scene materials vector.
    scene.materials = { lightRed, lightGreen, lightBlue, black, gray };
...
// Define point light sources.
    ...
    PointLightSource light0 = { Vector3d(100.0, 120.0, 10.0), Color(1.0f, 1.0f, 1.0f) * 0.95f };
    PointLightSource light1 = { Vector3d(5.0, 80.0, 60.0), Color(1.0f, 1.0f, 1.0f) * 0.8f };
...
// Define surface primitives.
    ...
    auto rightVertPlane = new Plane( 0.0, 0.0, 1.0, 0.0, scene.materials[3] ); // Right vertical plane.
    auto bigSphere =  new Sphere( Vector3d( 40.0, 20.0, 42.0 ), 22.0, scene.materials[0] ); // Big sphere.
    auto smallSphere = new Sphere( Vector3d( 75.0, 10.0, 40.0 ), 12.0, scene.materials[1] ); // Small sphere.
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            makePyramid(offX * i, offY * j, scene);
        }
    }
...
void makePyramid(double x, double y, Scene &scene) {
    Vector3d apex = Vector3d(20.0 + x, 35.0, 20.0 + y);

    // t1-4 for a pyramid
    auto t1 = new Triangle(Vector3d(0.0 + x, 0.0, 0.0 + y),
                           Vector3d(0.0 + x, 0.0, 40.0 + y),
                           apex,
                           scene.materials[2]);

    auto t2 = new Triangle(Vector3d(0.0 + x, 0.0, 40.0 + y),
                           Vector3d(40.0 + x, 0.0, 40.0 + y),
                           apex,
                           scene.materials[2]);

    auto t3 = new Triangle(Vector3d(40.0 + x, 0.0, 40.0 + y),
                           Vector3d(40.0 + x, 0.0, 0.0 + y),
                           apex,
                           scene.materials[2]);

    auto t4 = new Triangle(Vector3d(40.0 + x, 0.0, 0.0 + y),
                           Vector3d(0.0 + x, 0.0, 0.0 + y),
                           apex,
                           scene.materials[2]);

    auto tip = new Sphere(apex, 8.0, scene.materials[2] ); // Small sphere.

    scene.surfaces.push_back(t1);
    scene.surfaces.push_back(t2);
    scene.surfaces.push_back(t3);
    scene.surfaces.push_back(t4);
    scene.surfaces.push_back(tip);
}
